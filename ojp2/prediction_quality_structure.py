...
class PredictionQualityStructure:
    ...
    :ivar prediction_level: An approxiimate characterisation of
        prediction quality as one of five values. (since SIRI 2.0)
    :ivar percentile: Percentile associated with range as specified by
        lower and upper bound. (since SIRI 2.0)
    :ivar lower_time_limit: Lower bound on time of prediction for
        confidence level if different from defaults. (since SIRI 2.0)
    :ivar higher_time_limit: Upper bound on time of prediction for
        confidence level if different from defaults. (since SIRI 2.0)
    """
