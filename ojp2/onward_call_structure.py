...
class OnwardCallStructure(AbstractMonitoredCallStructure):
    ...
    :ivar latest_expected_arrival_time: Latest time at which a VEHICLE
        will arrive at stop. (since SIRI 2.1)
    :ivar expected_arrival_prediction_quality: Prediction quality,
        either as approximate level, or more quantitative percentile
        range of predictions that will fall within a given range of
        times. (since SIRI 2.0)
    :ivar arrival_prediction_unknown: If the producer is (temporarily)
        not able to deliver real-time predictions (e.g. because of a
        connection loss), he has various options (and combinations of
        them) to inform the consumer: (i) set Monitored to 'false' (ii)
        transmit ExpectedArrivalTime together with
        "PredictionInaccurate=true" (iii) drop/omit ExpectedArrivalTime
        (iv) set ArrivalStatus to 'noReport' However, this might
        introduce ambiguity: e.g. option (iii) might be interpreted as
        "on time" by one consumer, but also as "indefinite delay" by
        another consumer. To avoid this, the producer should transmit
        xxxPredictionUnknown instead of any expected times. (since SIRI
        2.1)
    :ivar arrival_status:
    ...
    :ivar provisional_expected_departure_time: Expected departure time
        ...
        leave earler than expected departure time (since SIRI 2.0).
    ...
    :ivar expected_departure_prediction_quality: Prediction quality,
        ...
        range of predictions that will fall within a given range of
        times. (since SIRI 2.0)
    :ivar departure_prediction_unknown: If the producer is (temporarily)
        not able to deliver real-time predictions (e.g. because of a
        connection loss), he has various options (and combinations of
        them) to inform the consumer: (i) set Monitored to 'false' (ii)
        transmit ExpectedDepartureTime together with
        "PredictionInaccurate=true" (iii) drop/omit
        ExpectedDepartureTime (iv) set DepartureStatus to 'noReport'
        However, this might introduce ambiguity: e.g. option (iii) might
        be interpreted as "on time" by one consumer, but also as
        "indefinite delay" by another consumer. To avoid this, the
        producer should transmit xxxPredictionUnknown instead of any
        expected times. (since SIRI 2.1)
    ...
    expected_arrival_prediction_quality: Optional[
