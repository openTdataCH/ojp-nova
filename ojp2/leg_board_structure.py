...
class LegBoardStructure:
    """
    Describes the situation at a stop or station at which the passenger boards a
    Leg of a trip including time-related information.
    ...
    :ivar geo_position:
    :ivar service_arrival: Contains ARRIVAL times (timetable, recorded,
        estimated, timing bands estimated) and the ARRIVAL formation.
    :ivar service_departure: Contains DEPARTURE times (timetable,
        recorded, estimated, timing bands estimated) and the DEPARTURE
        formation.
    :ivar distributor_interchange_id: Interchange identifier of the
        distributing line/service at its boarding. This is not a
        reference. This identifier is used to recognize in a distributed
        environment (e.g., EU-Spirit), that two systems refer to the
        same line (or service) while using their own internal
        references. In EU-Spirit this is used to decide whether an
        interchange is in fact a stay-seated scenario (aka "line ID").
        See https://eu-spirit.eu/
    ...
    distributor_interchange_id: Optional[str] = field(
        default=None,
        metadata={
            "name": "DistributorInterchangeId",
            "type": "Element",
            "namespace": "http://www.vdv.de/ojp",
        },
    )
