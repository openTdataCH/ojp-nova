...
@dataclass
class ParallelServiceStructure:
    """Services running combined with at least parts of this journey, e.g., wing
    trains.

    The contained stop sequence interval refers to the original journey.

    :ivar from_stop_sequence_number: Stop sequence number, where the
        interval begins (inclusive). If missing, the interval begins at
        the beginning of the leg (or at the beginning of the journey for
        uses in StopEvents and TripInfo).
    :ivar to_stop_sequence_number: Stop sequence number, where the
        interval ends (inclusive). If missing, the interval ends at the
        end of the leg (or at the end of the journey for uses in
        StopEvents and TripInfo).
    :ivar journey_ref:
    ...
    from_stop_sequence_number: Optional[int] = field(
        default=None,
        metadata={
            "name": "FromStopSequenceNumber",
            "type": "Element",
            "namespace": "http://www.vdv.de/ojp",
        },
    )
    to_stop_sequence_number: Optional[int] = field(
        default=None,
        metadata={
            "name": "ToStopSequenceNumber",
            "type": "Element",
            "namespace": "http://www.vdv.de/ojp",
        },
    )
