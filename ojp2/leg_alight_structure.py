...
class LegAlightStructure:
    ...
    :ivar service_arrival: describes the arrival situation at the leg
        alight stop point (group of attributes of TIMETABLED PASSING
        TIME, ESTIMATED PASSING TIME, OBSERVED PASSING TIME)
    :ivar service_departure: describes the departure situation at this
        leg alight stop point (empty for last leg) (group of attributes
        of TIMETABLED PASSING TIME, ESTIMATED PASSING TIME, OBSERVED
        PASSING TIME)
    :ivar feeder_interchange_id: Interchange identifier of the feeding
        line/service at its alighting. This is not a reference. This
        identifier is used to recognize in a distributed environment
        (e.g., EU-Spirit), that two systems refer to the same line (or
        service) while using their own internal references. In EU-Spirit
        this is used to decide whether an interchange is in fact a stay-
        seated scenario (aka "line id"). See https://eu-spirit.eu/
    ...
    feeder_interchange_id: Optional[str] = field(
        default=None,
        metadata={
            "name": "FeederInterchangeId",
            "type": "Element",
            "namespace": "http://www.vdv.de/ojp",
        },
    )
