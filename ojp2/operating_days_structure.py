...
class OperatingDaysStructure:
    ...
    This is useful, when the trip can be made during several days. Often
    users search in "general" without setting a date and then refine
    with the actual date. By using OperatingDaysStructure the result can
    directly be reused without an additional OJPTripRequest.
    ...
    :ivar pattern: Bit pattern for operating days between start date and
        ...
        that an event happens on the day that is represented by the bit
        position.
    """
