...
class TripProblemTypeEnumeration(Enum):
    ...
    :cvar TRIP_DATETIMEERROR: The requested date and/or time do not make
        sense.
    :cvar TRIP_TIMEWINDOWTOOLARGE: The requested time window is too
        large.
    ...
    :cvar TRIP_VIAEQUIVALENT: One of the requested via stop places has
        been replaced by an equivalent stop place.
    :cvar TRIP_REALTIMEINCOMPLETE: There is no real-time information
    ...
    :cvar TRIP_ITMODECHANGED: The mode of individual transport specified
        ...
        trip could be found. Usually, this means taking a taxi instead
        of walking.
    :cvar TRIP_INCONVENIENTWAITING: The trip plan in this trip result
        contains a long waiting time.
    :cvar TRIP_RESTRICTEDRESULTSDROPPED: Used for warnings, when
        possible/better results were dropped, because of the criteria
        were not used (e.g., private services, offer only available for
        seniors).
    ...
    TRIP_TIMEWINDOWTOOLARGE = "TRIP_TIMEWINDOWTOOLARGE"
    ...
    TRIP_RESTRICTEDRESULTSDROPPED = "TRIP_RESTRICTEDRESULTSDROPPED"
